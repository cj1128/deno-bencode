...
Deno.test("should sort dictionaries", () => {
  ...
  assertEquals(td.decode(encode(data)), 'd7:integeri12345e6:string11:Hello Worlde')
})
...
Deno.test('should be able to encode a positive integer', () => {
  assertEquals(td.decode(encode(123)), 'i123e')
})

Deno.test('should be able to encode a negative integer', () => {
  assertEquals(td.decode(encode(-123)), 'i-123e')
})

Deno.test('should be able to encode a positive float (as int)', () => {
  assertEquals(td.decode(encode(123.5)), 'i123e')
})

Deno.test('should be able to encode a negative float (as int)', () => {
  assertEquals(td.decode(encode(-123.5)), 'i-123e')
})
