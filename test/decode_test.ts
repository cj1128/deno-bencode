...
Deno.test('should be throw an error when trying to decode a broken integer', () => {
  assertThrows(() => {
    decode('i12+3e')
  }, Error, 'not a number')
...
Deno.test('should be able to decode a float (as int)', () => {
  assertEquals(decode('i12.3e'), 12)
  assertEquals(decode('i-12.3e'), -12)
})

Deno.test('should be throw an error when trying to decode a broken float', () => {
  assertThrows(() => {
    decode('i1+2.3e')
  }, Error, 'not a number')
  assertThrows(() => {
